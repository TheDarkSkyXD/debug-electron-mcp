...
The former HTTP entry imported the v1 `StreamableHTTPServerTransport`, detected `initialize`, created and stored transports by session ID, and called `connect` for each session. Those responsibilities conflicted with the 2026 protocol's removed handshake and protocol-level session. [Current HTTP entry](../../src/transport/http-server.ts) [official changelog](https://modelcontextprotocol.io/specification/2026-07-28/changelog)
