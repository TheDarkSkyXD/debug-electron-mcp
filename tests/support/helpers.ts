...
export class TestHelpers {
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkReady = async () => {
        try {
          ...
          if (response.ok) {
            logger.info(`✅ Test Electron app ready on port ${app.port}`);
            resolve();
            return;
          }
        } catch {
  ...
  static async cleanup(options: CleanupOptions = {}): Promise<void> {
    const { removeLogsDir = true, removeTempDir = true } = options;
   ...
   */
  static createMCPRequest(toolName: string, args: Record<string, unknown> = {}) {
    return {
